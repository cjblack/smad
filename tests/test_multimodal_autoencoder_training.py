...
from smad.training.train import train_model_packed, auto_regressive_fine_tuning, train_multimodal_model_kinematics_inspired
...
def test_training():
    ...
    """
    config_file = 'smad_multimodal_autoencoder.yml'
    ...
    batch_size=32
    train_loader = create_data_loader(data=dataset_train, shuffle=False, batch_size=batch_size, collate_fn=collate_fn)
    ...
    data_dict = {'ephys': train_loader, 'kinematic': train_loader}
    modalities = ['ephys','kinematic']
    dataloaders = [train_loader, train_loader]
    modal_loader = [modalities,dataloaders]
    model, training_info, criterion, optimizer, device = train_multimodal_model_kinematics_inspired(config_file, modal_loader, noise=0.05)
